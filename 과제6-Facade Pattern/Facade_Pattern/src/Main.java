...
public class Main {
    public static void main(String[] args) {
        ...
        */
        IDE ide = new IDE("ArrayProject.exe", preprocessor, compiler, linker);
        SourceCode[] sourceCodes = new SourceCode[fileNames.length];
        for (int i = 0; i < fileNames.length; i++) {
            sourceCodes[i] = new SourceCode(fileNames[i]);;
        }
        ide.build(sourceCodes);

        ide.createProject("ListProject.exe");
        List<SourceCode> list =  new ArrayList<>(Arrays.asList(sourceCodes));
        ide.build(list);
